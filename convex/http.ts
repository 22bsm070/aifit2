...
http.route({
    ...
    handler:httpAction(async (ctx,request) => {
        ...
        //user.created to do
        return new Response("webhoooks processed successfully",{status:200});
...
http.route({
    ...
    handler: httpAction(async(_ctx,request) => {
        ...
        Height: ${height}
        Weight: ${weight}
        Injuries or limitations: ${ past_injuries}
        Available days for workout: ${available_workout_days}
        ...
        Weight: ${weight}
        Fitness goal: ${fitness_goals}
        Dietary restrictions: ${any_dietary_restriction}
        ...
        //validate the output coming from ai
        ...
        console.log('About to save plan for user:', user_id);
        console.log('Workout plan:', JSON.stringify(workoutPlan, null, 2));
        console.log('Diet plan:', JSON.stringify(dietPlan, null, 2));
        
        const planId = await _ctx.runMutation(api.plans.createPlan, {
            userId: user_id,
            dietPlan,
            isActive: true,
            workoutPlan,
